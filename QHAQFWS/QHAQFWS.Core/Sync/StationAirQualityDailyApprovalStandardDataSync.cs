...
namespace QHAQFWS.Core.Sync
{
    public class StationAirQualityDailyApprovalStandardDataSync : SyncBase<Air_StationAQIHistory_Day_App_Std>
    {
        ...
        protected override List<Air_StationAQIHistory_Day_App_Std> GetSyncData(DateTime time)
        {
            ...
            {
                SiteDaily[] srcList = client.GetSiteDailyData(time, time, (int)AirQualityDataType.ApprovalStandard);
                List<Air_StationAQIHistory_Day_App_Std> existList = Model.Air_StationAQIHistory_Day_App_Std.Where(o => o.TimePoint == time).ToList();
        ...
        {
            IQueryable<Air_StationAQIHistory_Day_App_Std> list = Model.Air_StationAQIHistory_Day_App_Std.Where(o => o.TimePoint == time);
            if (list.Any())
            {
                Model.Air_StationAQIHistory_Day_App_Std.RemoveRange(list);
            }
        }
    }
}
